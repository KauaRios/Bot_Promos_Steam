...
# Criamos o "bastão da fala" para o banco de dados
...
def processar_linha(linha):
    ...
    if type(resultado) is dict:
        # A MÁGICA AQUI: O operário pede permissão (pega o bastão) para escrever no banco
        with lock_banco:
...
def populardb():
    # 1. Lemos o arquivo inteiro de uma vez
    ...
    # 2. Criamos uma equipe de 5 operários (threads)
    # Não coloque um número muito alto (como 50), senão a Steam percebe e dá ban.
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        # O executor.map divide a sua lista entre os operários automaticamente
